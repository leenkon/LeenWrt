...
## 适用场景
- **带 vermagic 的 kmod**（内核模块，如网卡驱动、kmod-tun、fwx 等）：
  官方 release 源的 kmod 与自编译内核的 vermagic 不一致，`insmod` 会拒绝加载，
  因此必须随固件编译进镜像（由 `LeenWrtBuilder.yml` (workflow) 在 `.config` 追加 `CONFIG_PACKAGE_kmod-*` 段 + `make defconfig` 生成）或作为同源编译的 `.apk` 离线装。
...
- fwx 应用过滤栈：`kmod-fwx` 由 `LeenWrtBuilder.yml` 构建期调用 `scripts/sync-fwx.sh` 按钉死 SHA 从 upstream 动态拉取（不 vendored 进仓库）；`fwxd`/`libfwx_common` 由本地 vendored feed `feeds/fwx/` 提供；14 个 `luci-app-fwx-*` 界面包通过 `feeds/25.12.conf` 中的 `src-git fwxluci` 跟随上游。三者均随固件编译进镜像，无需放这里。若拿到自编译 / 手动上传的可信 `.apk`（含 `kmod-fwx*.apk`），放这里即可在首启离线安装。
